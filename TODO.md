...
- Make import classifications enforceable. `allowedWithLimits(fps:networkBudget:warnings:)` is currently surfaced to users, but runtime enforcement is incomplete.
...
- Strengthen monitor identity persistence beyond raw `CGDirectDisplayID`, using display metadata where available and falling back gracefully.
- Add runtime network policy for web wallpapers: block external navigation by default and enforce any configured allowlist or request budget.
